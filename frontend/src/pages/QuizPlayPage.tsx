...
import { awardXp } from "@/lib/ranking"
...
function getGuestId(): string {
  try {
    let id = localStorage.getItem("histori_guest_id")
    if (!id) {
      id = `guest_${Math.random().toString(36).slice(2, 10)}`
      localStorage.setItem("histori_guest_id", id)
    }
    return id
  } catch {
    return "guest_anon"
...
export default function QuizPlayPage() {
  ...
  const finishQuiz = useCallback(() => {
    ...
    // Global ranking XP — always (guest or logged-in) so /leaderboard fills up
