  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
      ...
      <form onSubmit={onSubmit} className="flex gap-2 mb-8">
        ...
        <button type="submit" className="px-5 rounded-lg bg-primary text-black font-medium hover:opacity-90">
          {t("common.search")}
        </button>
      </form>
      {!q && <p className="text-muted">{t("search.hint")}</p>}
      {q && hits.length === 0 && <p className="text-muted">{t("search.noResults")}</p>}
